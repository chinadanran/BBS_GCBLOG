...
class Mypage:
    def __init__(self, page_num, all_data_amount, url_prefix, request, per_page_data=10, page_show_tags=9):
        ...
        self.querydict = copy.deepcopy(request.GET)
    ...
    def ret_html(self):
        ...
        first_page_tag = '<li><a href="/{0}/?{1}">首页</a></li>'.format(self.prefix, self.querydict.urlencode())
        self.querydict['page'] = self.total_page_num
        last_page_tag = '<li><a href="/{0}/?{1}">尾页</a></li>'.format(self.prefix, self.querydict.urlencode())
        self.querydict['page'] = self.page_num - 1
        front_page_tag = '<li><a href="/{0}/?{1}">&laquo;</a></li>'.format(self.prefix, self.querydict.urlencode())
        self.querydict['page'] = self.page_num + 1
        next_page_tag = '<li><a href="/{0}/?{1}">&raquo;</a></li>'.format(self.prefix, self.querydict.urlencode())

        page_tag_html = ''
        for i in range(show_tags_left, show_tags_right + 1):
            self.querydict['page'] = i
            if i == self.page_num:
                page_tag_html += '<li class="active"><a href="/{0}/?{2}">{1}</a></li>'.format(self.prefix, i,
                                                                                              self.querydict.urlencode())
            else:
                page_tag_html += '<li><a href="/{0}/?{2}">{1}</a></li>'.format(self.prefix, i,
                                                                               self.querydict.urlencode())
